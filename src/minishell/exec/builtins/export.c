...
/*   Updated: 2025/05/21 18:35:42 by cboma-ya         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	print_export(t_data *data)
{
	...
	{
		node = data->export_list->head;
		while (node)
		{
			export_var = node->content;
		...
		/*je veux dire ici que si c'est pas dans l'env, mais que c'est
			...
			Je crée un argleau export ? pas besoin de stuct vu que je veux juste 
...
void	ft_export(t_segment_content *content, t_data *data)
{
	...
	else
	{
		...
		while (arg[i])
		{
			if (valid_export(arg[i]) == true)
			{
				get_name_and_value(arg[i], tmp_env, tmp_export, data);	
				if (ft_strchr(arg[i], '='))
