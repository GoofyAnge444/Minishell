...
/*   Updated: 2025/05/30 13:03:29 by eazard           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	set_path(char **path, t_cmd_content *content, t_data *data)
{
	if (!content->cmd_args[1])
	{
		*path = ft_getenv("HOME", data);
		if (!path)
			return (set_last_exit_code(data, 1),
...
void	ft_cd(t_cmd_content *content, t_data *data)
{
	...
	if (ft_tablen(content->cmd_args) > 2)
		return (set_last_exit_code(data, 1),
			(void)ft_putstr_fd("mishell: cd: too many arguments\n", 2));
	set_path(&path, content, data);
	old_pwd = getcwd(NULL, 0);
	if (!old_pwd)
		return (set_last_exit_code(data, 1), fatal_error_clean_exit(data,
				MALLOC_FAILURE));
	ret = chdir(path);
	if (ret == -1)
		return (set_last_exit_code(data, 1), free(old_pwd),
			perror("mishell: cd"));
	new_pwd = getcwd(NULL, 0);
	if (!new_pwd)
		return (free(old_pwd), set_last_exit_code(data, 1),
