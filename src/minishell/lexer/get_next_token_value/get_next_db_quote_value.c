...
/*   Updated: 2025/02/11 15:39:34 by cboma-ya         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static size_t	get_db_quote_token_len(t_data *data)
{
	...
	i = 0;
	while (break_condition_db_quote(actual_character_offset(data, i)))
		i++;
	return (i);
}
...
char	*get_next_db_quote_value(t_data *data)
{
	...
	token_value = ft_calloc(get_db_quote_token_len(data), 1);
	if (!token_value)
